...
sched_pol_recursive_all(){
		...
		#on_empty_input_print_and_exit cron_all_pkgs "Error: The Policy-Package name can not be empty."

		#cron_target_fw_response=$( mgmt_cli show package name cron_all_pkgs -u $usr -p $pass > /dev/null ) 
		#on_error_print_and_exit "Error: The Policy-Package verification process has failed : \n $cron_target_fw_response"
        
        echo -e "Will be schedule a recursive Policy Installation of Every Policy Package created on the system, including all firewalls that belongs to them" ;
        echo -e "Please, choose one of the following predefined options to be applied on the system.\n" ;
		echo "-------------------- Frequency --------------------------------"
		...
		if [[ ( $opt_cron == "D2" ) || ( $opt_cron == "d2" ) ]]; then
			...
			echo "The current date/time system information is: $(date)"
			...
			echo  "$m1,$m2 $h1,$h2 * * *    $(pwd)/$usr/recursive/$( date +%d-%m-%Y )/cron_all_pkgs_crontask_"$usr".sh 2> $(pwd)/$usr/logs/$( date +%d-%m-%Y )/stdout_recursive_task_$usuario-$(date +%d%m%Y ).txt"  >> /var/spool/cron/$USER 
		    cp -p $(pwd)/base/bse_cron_inst_pol_all.sh  $(pwd)/$usr/recursive/$( date +%d-%m-%Y )/cron_all_pkgs_crontask_"$usr".sh
		    echo "";
		    echo -e "===============================================================================================";
			...
			echo  "$m1,$m2 $h1,$h2 * * 6,0    $(pwd)/$usr/recursive/$( date +%d-%m-%Y )/cron_all_pkgs_crontask_"$usr".sh 2> $(pwd)/$usr/logs/$( date +%d-%m-%Y )/recursive_task_$usuario-$(date +%d%m%Y ).txt"  >> /var/spool/cron/$USER 
		    cp -p $(pwd)/base/bse_cron_inst_pol_all.sh  $(pwd)/$usr/recursive/$( date +%d-%m-%Y )/cron_all_pkgs_crontask_"$usr".sh
		    echo "";
		    echo -e "===============================================================================================";
			...
			echo  "0 0 * * *    $(pwd)/$usr/recursive/$( date +%d-%m-%Y )/cron_all_pkgs_crontask_"$usr".sh 2> $(pwd)/$usr/logs/$( date +%d-%m-%Y )/recursive_task_$usuario-$(date +%d%m%Y ).txt"  >> /var/spool/cron/$USER 
		    cp -p $(pwd)/base/bse_cron_inst_pol_all.sh  $(pwd)/$usr/recursive/$( date +%d-%m-%Y )/cron_all_pkgs_crontask_"$usr".sh
		    echo "";
		    echo -e "===============================================================================================";
