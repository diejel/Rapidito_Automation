...
JQ=${CPDIR}/jq/jq
usuario=$(echo $0 | sed 's/.sh$//i' | awk -F '[-]' '{print $9}')
...
do    
    ...
    echo "Installing policy in Firewall ${mat_pol_pkg[i+1]} ";
    echo "Progress: [ $num / $den ]";
    pub_ip_fw=$(mgmt_cli show simple-gateway name $mat_target_fw -u $usr -p $pass --version 1.1 | grep -i 'ipv4-address' | egrep -v '^10.*|^127.*|^192.*|^169.*|^172.*' | cut -d":" -f2 | cut -d"\"" -f2 | tr '\n' ' ' | cut -d' ' -f1)
    resp_ip_fw=$( nc -z -v -w 2 $pub_ip_fw $port )
    if [[ $resp_ip_fw -eq 0 ]] ; then
          mgmt_cli install-policy policy-package ${mat_pol_pkg[$i]} access true threat-prevention true targets.1 $mat_target_fw -u $usr -p $pass | tee -a /var/rapidito/$usuario/logs/scheduled_task_$usuario-$(date +%d%m%Y ).txt 
          sleep 3
    else
          :
    fi
